...
def _main():
    ...
    obj = proctree(tree)
    write_result(obj,output_directory)
...
def write_result(obj,output_directory):
...
class Convert(object):
    ...
    def _special_para(self,el):
        """
        Special case handling, for when inline code blocks are within a paragraph. This strips
        out text formatting, while preserving the code block formatting.
        """
        lines = el.text.split('\n')
        text = ""
        acc = ""
        ...
        for line in lines:
            ...
            if (skipfirst or (skipline and line <> '')):
                text += line + '\n'
                skipindent = len(line) - len(line.lstrip())
                # print "Establishing indent on \n%s\nof %s" % (line,skipindent)
                skipline = False
                skipfirst = False
                skiprest = True
                ...
                if (len(line) - len(line.lstrip()) < skipindent and line <> ''):
                    # print "Done skipping on %s" % line
                    skiprest = False
                    skipindent = 0
                    acc = '\n' + line + '\n'
                else:
                ...
                # print "Starting skip"
                text += textwrap.fill(' '.join(acc.split()).strip()) + '\n\n'
                acc = ""
                text += line + '\n'
                skipline = True
            else:
    ...
    def _concat(self, el):
        ...
        if el.text is not None:
            if str(el.tag) == 'para':
                text = self._special_para(el)
                # text = textwrap.fill(text)+ '\n'
            else:
                text = el.text
            s += self._remove_indent_and_escape(text)
        for i in el.getchildren():
            s += self._conv(i)
    ...
    def e_itemizedlist(self, el, bullet="-"):
        ...
        for i in el.getchildren():
            indent = len(bullet + ' ')
            s += self._indent(i, indent, bullet+" ")
        return s + "\n\n"
    ...
    def e_informaltable(self,el):
        """
        We are going to generate a list-table.
        """
        text = ".. list-table::\n"

        names = []
        widths = []
        for c in el.find('tgroup').findall('colspec'):
            names.append(c.attrib['colname'])
            widths.append(c.attrib['colwidth'])

        text += '   :widths: ' + ' '.join(widths) + '\n'
        text += '   :header-rows: 1\n\n'
        ...
        headRest    = "      - "
        indentBlank = "        "
        wrapper = textwrap.TextWrapper(initial_indent   = headFirst,
                                       subsequent_indent= indentBlank,
                                       replace_whitespace = True,
                                       width = 70)

        heading = el.find('tgroup').find('thead')
        if heading is not None:
            for col in heading.find('row').findall('entry'):
                text += wrapper.fill(col.text) + '\n'
                wrapper.initial_indent = headRest
        else:
            for n in names:
                text += wrapper.fill(n) + '\n'
                wrapper.initial_indent = headRest
        ...
        for row in el.find('tgroup').find('tbody').findall('row'):
            wrapper.initial_indent = headFirst
            for ent in row.findall('entry'):
                text += wrapper.fill(' '.join(ent.text.split())) + '\n'
                wrapper.initial_indent = headRest

        return text

    def e_procedure(self,el):

        text = ''
        for step in el.findall('step'):
            text += self._indent(step, 3, '#. ')
