...
(defun aux-parser-get-values (session loc min-size prior-post-index)
  (let ((pre-index      (parser-location-index loc))
	...
	(dolist (node max-nodes)
	  (when (eq (parser-node-rule node) start-rule)
	    (incf toplevel-count)
	    (push (list (parser-location-position (svref locations pre-index))
		  ...
		  alternative-results)))
	(cond ((null alternative-results)
	       (report-gap session pre-index max-index)
	       '(when (parse-session-report-gaps? session)
		 (dolist (node max-nodes)
		   (format t "~&Rule = ~S~%" (parser-node-rule node)))))
	      ((not (null (cdr alternative-results)))
...
(defun report-ambiguity (session start end alternative-results)
  ...
  (let* ((start-blc (parser-location-position (svref (parse-session-locations session) start)))
    ...
    ;; (dolist (x alternative-results) (print x))
    (format t "~&----~%")
    ;; (comment "============================================================================")
    ))
