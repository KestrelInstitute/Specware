;;; slang-mode.el. Major mode for editing Specware Slang
;;; Adapted from sml-mode
;; Copyright (C) 1989, Lars Bo Nielsen; 1994, Matthew J. Morley

;; (autoload 'slang-mode "slang-mode" "Major mode for editing SLang programs." t)
;;
;; Files ending in ".sl" are considered by emacs hereafter to
;; be SLang source, so put their buffers into slang-mode automatically

;;; (setq auto-mode-alist
;;;       (cons '("\\.sl$" . slang-mode) auto-mode-alist))

;; Here's an example of setting things up in the slang-mode-hook:

;; (setq slang-mode-hook
;;       '(lambda() "SLang mode hacks"
;;          (setq sl-indent-level 2         ; conserve on horiz. space
;;                indent-tabs-mode nil)))    ; whatever

;; slang-mode-hook is run whenever a new slang-mode buffer is created.
;; There is an slang-load-hook too, which is only run when this file is
...
;; Finally, there is also an inferior-slang-mode-hook -- see
;; sl-proc.el. For more information consult the mode's *info* tree.

;;; VERSION STRING

(defconst slang-mode-version-string
  "slang-mode, Version 0.1")
...
(defvar sl-indent-level 2
  "*Indentation of blocks in SLang.")
...
(defvar slang-mode-hook nil
  "*This hook is run when slang-mode is loaded, or a new slang-mode buffer created.
This is a good place to put your preferred key bindings.")

(defvar slang-load-hook nil
  "*This hook is only run when slang-mode is loaded.")
...
(defun install-sl-keybindings (map)
  ...
  (define-key map "\M-."     'slang-meta-point)
  ...
  ;; Process commands added to slang-mode-map -- these should autoload
...
(defvar slang-mode-map nil "The mode map used in slang-mode.")
(cond ((not slang-mode-map)
       (setq slang-mode-map (make-sparse-keymap))
       (install-sl-keybindings slang-mode-map)))
...
(defun slang-mode-version ()
  "This file's version number (slang-mode)."
  (interactive)
  (message slang-mode-version-string))

(defvar slang-mode-syntax-table nil "The syntax table used in slang-mode.")
(if slang-mode-syntax-table
    ()
  (setq slang-mode-syntax-table (make-syntax-table))
  ;; Set everything to be "." (punctuation) except for [A-Za-z0-9],
  ;; which will default to "w" (word-constituent).
  (let ((i 0))
    (while (< i ?0)
      (modify-syntax-entry i "." slang-mode-syntax-table)
      (setq i (1+ i)))
    (setq i (1+ ?9))
    (while (< i ?A)
      (modify-syntax-entry i "." slang-mode-syntax-table)
  ...
  (modify-syntax-entry ?\(      "()5"   slang-mode-syntax-table)
  (modify-syntax-entry ?\)      ")(8"   slang-mode-syntax-table)
  (modify-syntax-entry ?\[      "(]"    slang-mode-syntax-table)
  (modify-syntax-entry ?\]      ")["    slang-mode-syntax-table)
  (modify-syntax-entry ?{       "(}"    slang-mode-syntax-table)
  (modify-syntax-entry ?}       "){"    slang-mode-syntax-table)
  (modify-syntax-entry ?\*      ". 67"  slang-mode-syntax-table)
  (modify-syntax-entry ?\"      "\""    slang-mode-syntax-table)
  (modify-syntax-entry ?        " "     slang-mode-syntax-table)
  (modify-syntax-entry ?\t      " "     slang-mode-syntax-table)
  (modify-syntax-entry ?\%      "<   "     slang-mode-syntax-table)
  (modify-syntax-entry ?\n      ">   "     slang-mode-syntax-table)
  (modify-syntax-entry ?\f      " "     slang-mode-syntax-table)
  (modify-syntax-entry ?\'      "w"     slang-mode-syntax-table)
  (modify-syntax-entry ?\_      "w"     slang-mode-syntax-table)
  (modify-syntax-entry ?\-      "w"     slang-mode-syntax-table)
  (modify-syntax-entry ?\?      "w"     slang-mode-syntax-table))

(defun slang-mode ()
  "Major mode for editing SLang code.
Tab indents for SLang code.
Comments are delimited with (* ... *).
Blank lines and form-feeds separate paragraphs.
Delete converts tabs to spaces as it moves back.

For information on running an inferior SLang process, see the documentation
for inferior-slang-mode (set this up with \\[sl]).

Customization: Entry to this mode runs the hooks on slang-mode-hook.
...
\\{slang-mode-map}"
  ...
  (slang-mode-variables)
  (use-local-map slang-mode-map)
  (setq major-mode 'slang-mode)
  (setq mode-name "SLang")
  (run-hooks 'slang-mode-hook))           ; Run the hook

;; What is the deal? This is a symbol, but it's also defined as a var?

(defvar slang-mode-abbrev-table nil "*SLang mode abbrev table (default nil)")

(defun slang-mode-variables ()
  (set-syntax-table slang-mode-syntax-table)
  (setq local-abbrev-table slang-mode-abbrev-table)
  ...
  (setq font-lock-fontify-region-function
        'slang-font-lock-fontify-region-function)
  ;;
...
(defun slang-font-lock-fontify-region-function (beg end &optional loudvar)
  ;(slang-font-lock-fontify-tex beg end)
  (font-lock-default-fontify-region beg end loudvar)
  (slang-font-lock-fontify-tex beg end)
)

(defun slang-font-lock-fontify-tex (beg end)
  (goto-char beg)
...
(defun sl-indent-line ()
  "Indent current line of SLang code."
  (interactive)
...
(defun sl-insert-form ()
  "Interactive short-cut to insert a common SLang form."
  (interactive)
...
(defun slang-meta-point (name)
  (interactive (list (car (sl::get-default-symbol "SLang locate source" t t))))
  (let* ((pr (find-spec-info name))
	 (spec (car pr))
	 (sym (cadr pr)))
    ...
    (let ((sym (if (equal (substring sym 0 2) "|!")
		   (substring sym 2 -1)
		 sym)))
      (let ((file (fi:eval-in-lisp (format "(emacs::get-source-file-info %S %S)"
					   (upcase spec) sym))))
	(message "")
	(if (member file '("NO-SPEC" "NO-SYMBOL" "NO-FILE"))
	    (error "Can't find definition of %s: %s" name file)
	  (push-mark (point))
	  (let ((buf
	  ...
	  (goto-char 0)
	  (re-search-forward (concat "\\bspec\\s-+" spec "\\W") nil t)
	  (let ((qsym (regexp-quote sym)))
	    (or (if (null current-prefix-arg)
		    (or (re-search-forward (concat "\\bdef\\s-+" qsym "\\W") nil t)
  ...
  (let ((colon-pos (fi::lisp-find-char ?: name)))
	...
	(let ((dot-pos (fi::lisp-find-char ?. name)))
	  (if dot-pos			; has a package
	    ...
	    (list (if (eq major-mode 'fi:inferior-common-lisp-mode)
		      (spec-from-fi:package)
		    (find-containing-spec))
		  name))))))
...
(defun sl::get-default-symbol (prompt &optional up-p ignore-keywords)
  (let ((symbol-at-point (sl::get-symbol-at-point up-p)))
...
(defun sl::get-symbol-at-point (&optional up-p)
  (let ((symbol
	 (cond
	  ((looking-at "\\sw\\|\\s_\\|\\.")
	   (save-excursion
...
(add-hook 'slang-load-hook 'slang-mode-version t)

(run-hooks 'slang-load-hook)

;;; slang-mode.el has just finished.
