...
(defun build-specware4 (in-current-dir?)
  ...
  (let* ((root-dir (if in-current-dir?
    ...
    (sw:eval-in-lisp (format "(setf (sys:getenv \"SWPATH\") %S)"
			     (concat (sw::normalize-filename root-dir) ":" slash-dir)))
...
;; This is called to start Planware. It is invoked by a command-line
;; argument to Xemacs. This spawns a Lisp process.
(defun run-planware (&optional in-current-dir?)
  (interactive "P")
  (let* ((root-dir (if in-current-dir?
		       (strip-final-slash (if (stringp in-current-dir?)
					      in-current-dir?
		     ...
		     (getenv "SPECWARE4")))
	 (bin-dir (concat root-dir
			  "/Applications/Planware/bin/"
			  (if *windows-system-p*
			      "windows"
			    (symbol-name system-type))))
	 (world-name (concat bin-dir "/Planware.dxl")))
