refine /Library/Base/List by {
 ...
 executable and reasonably efficient when translated to Lisp by the Specware
 code generator. We do not refine op definedOnInitialSegmentOfLength because it
 cannot be made executable (it involves checking a function at every natural
 number); this op is only used to express subtype constraints. Also, we do not
 refine ops whose definition in spec List is already executable and reasonably
 efficient (also considering the refined versions of the ops that are
 referenced in the original definitions). All the other ops are re-defined
 below. *)
