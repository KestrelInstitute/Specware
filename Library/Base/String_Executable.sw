...
proof isa Nat__natConvertible__1__obligation_refine_def
  ...
  * 1. Existence **
  ...
  apply (erule disjE, simp add: nat_of_char_def)+
  apply (simp add: add: nat_of_char_def)
  apply (frule Nat__natToString_Obligation_subtype,
         drule Nat__digitToString_Obligation_exhaustive)
  apply (erule disjE, simp (no_asm_simp) add: nat_of_char_def)+
  apply (simp (no_asm_simp) add: nat_of_char_def)
  * 2. Proper bounds *
