...
op [a] forall? (p: a -> Bool) (s:Set a) : Bool = (s <= p)

op [a] exists? (p: a -> Bool) (s:Set a) : Bool = nonEmpty? (s /\ p)

op [a] exists1? (p: a -> Bool) (s:Set a) : Bool = single? (s /\ p)

% filter set w.r.t. predicate:

op [a] filter (p: a -> Bool) (s:Set a) : Set a = s /\ p
...
type FSet a = (Set a | finite?)

type FSet1 a = (FSet a | nonEmpty?)
...
proof Isa induction
 ...
 assume ASM: "Fun_PD finite (p::'a Set__FSet \_Rightarrow bool)"
             "finite (s::'a set)"   "p {}" 
             "\_forall(s::'a set) (x::'a). finite s \_and p s \_longrightarrow p (insert x s)"
 thus "p (s::'a Set__FSet)"
...
op size : [a] FSet a -> Nat = the(size)
  (size empty = 0) &&
  (fa (s: FSet a, x:a) size (s <| x) = 1 + size (s - x))
...
op [a] powerf (s:Set a) : Set (FSet a) = power s /\ finite?
...
op [a] list2set (l:List a) : FSet a = fn x -> x in? l
