...
axioms sub_def: 
  "\<lbrakk>int n \<ge> 0; int n < int (length s)\<rbrakk> \<Longrightarrow> s ! n = id s ! n"
axioms substring_def: 
  "\<lbrakk>
   int j \<ge> 0; int i \<ge> 0; int i \<le> int j; 
   int j \<le> int (length s)\<rbrakk> \<Longrightarrow> 
...
axioms Nat__stringToNat_subtype_constr: 
  "\<lbrakk>Nat__natConvertible dom_stringToNat\<rbrakk> \<Longrightarrow> 
   int (Nat__stringToNat dom_stringToNat) \<ge> 0"
