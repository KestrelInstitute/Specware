Assert qualifying spec
  ...
  point is that, given a boolean expression (formula) `f', the expression
  `assert f' engenders a proof obligation that `f' is true. Given an
  expression `e[e0]' where we have singled out an occurrence of `e0' inside
  `e', if we expect `f' to be true at that occurrence of `e0', we can
  substitute `e[(assert f; e0)]', causing Specware to generate a proof
