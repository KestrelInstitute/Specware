...
lemma TwosComplement__toInt_inject [simp]:
 ...
 apply (simp_all only: convert_to_nat_2 of_nat_add int_int_eq)
  ...
  "\<lbrakk>x \<noteq> []\<rbrakk> \<Longrightarrow>
   ...
   apply (cases "k=0",
          auto simp add: TwosComplement__sign_def
                         ...
                         algebra_simps)
   apply (simp only: convert_to_nat_2 of_nat_add int_int_eq of_nat_power[symmetric],
          simp add: Bits__extendLeft_toNat_B1 [symmetric])
   proof -
     assume a1: "x \<noteq> []"
     assume "hd x = B1"
     then have "int (2 ^ length x + toNat (replicate k B1 @ x)) = int (toNat x + 2 ^ (k + length x))"
       using a1 by (simp add: Bits__extendLeft_toNat_B1 add.commute)
     then show "2 ^ length x + int (toNat (replicate k B1 @ x)) = int (toNat x) + 2 ^ (k + length x)"
       by auto
   qed
(*************************************************************************)
...
lemma TwosComplement__toInt_nat:
  "\<lbrakk>0 < length a; TwosComplement__toInt a = int i\<rbrakk>  \<Longrightarrow> toNat a = i"
   using TwosComplementInt(1) TwosComplement__nonNegative_p_iff_ge_0 by fastforce
...
lemma TwosComplement__toInt_hd_1:
  "\<lbrakk>0 < length a; TwosComplement__toInt a < 0\<rbrakk> \<Longrightarrow> hd a = B1"
   by (simp add: TwosComplementInt setToPred_def split: if_splits)
...
lemma TwosComplement__toInt_induct_neg [simp]:
  ...
  by (metis List__length_empty TwosComplement__minTCNumber_exists
            length_greater_0_conv not_less)
...
lemma TwosComplement__extendLeft_to_len_neg:
   ...
   apply (auto simp add: List__extendLeft_def,
          simp add: TwosComplementInt split: if_splits,
          simp only: int_power_simp algebra_simps,
          simp only: power2_int of_nat_add int_int_eq)
   apply (cut_tac k="length bs - len" in TwosComplement__extendLeft_to_len_neg_aux,
          auto)
   proof -
     assume a1: "len2 = length bs"
     assume a2: "int (toNat bs) + 2 ^ len = int i + 2 ^ length bs"
     assume a3: "\<forall>bsa. length bsa = length bs \<and> toNat bsa = i + 2 ^ length bsa - 2 ^ len \<longrightarrow> bsa = replicate (length bs - len) B1 @ toBits (i, len)"
     have "int (toNat bs + 2 ^ len) = int (i + 2 ^ len2)"
       using a2 a1 by simp
     then show "bs = replicate (length bs - len) B1 @ toBits (i, len)"
       using a3 a1 by (metis (no_types) add.commute add_diff_cancel_left' of_nat_eq_iff)
   qed
...
lemma TwosComplement_mod_pos:
  ...
  using TwosComplement__nonNegative_p_iff_ge_0 TwosComplement__toInt_def nat_mod_distrib nat_power_eq
        toBits_mod by force
