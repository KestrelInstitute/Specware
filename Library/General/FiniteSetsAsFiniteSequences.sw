FSet qualifying spec
  ...
  def toFSet finiteSet =  % not executable
    the (set) 
      choose[FSet] (fn seq -> 
      finiteSet = (fn x -> x in? seq)
      ) set

  op fromFSet : [a] FSet a -> FiniteSet a
  def fromFSet set =
    fn x -> choose[FSet] (fn seq -> x in? seq) set

  op in? infixl 20 : [a] a * FSet a -> Boolean
  def in? (x,set) = choose[FSet] (fn seq -> x in? seq) set
  ...
  def single? set = choose[FSet] (fn seq -> single? seq) set
  ...
  def theMember set = choose[FSet] (fn seq -> first seq) set
