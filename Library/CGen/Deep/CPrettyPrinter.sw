...
lines of the text, of course. Metaslang strings have no preset limit,
conceptually. In practice, Specware may run out of memory when processing
excessively long strings. Lisp does not have any preset limit on strings either;
in practice, if Lisp code is generated for this pretty printer, the Lisp runtime
may run out of memory if the generated strings is excessively long. A similar
observation applies to Haskell, C, and Java (these, together with Lisp, are all
the languages that can be generated by Specware). Given the memory size of
modern computers, the odds of Specware or Lisp or Haskell or C or Java running
out of memory due to an excessively long string are sufficiently low that we do
not impose any maximum length requirements on a string that represents
pretty-printed text. *)
...
(* The following ops increase and decrease the indentation level, i.e.  they
move the start of the non-blank text "in" and "out". If the level is 0,
decreasing it has no effect. Maybe in the future we should extend the monad to
also be an exception monad, and raise an exception when attempting to decrease
indentation level 0---this would easy debugging. *)
...
expressions that are part of our C subset. Thus, we do not an element for
...
the subsections of [ISO 6.5], e.g. the precedence of unary expression is higher
...
printing an additive expression, the left subexpression is printed with the ADD
auxiliary argument, while the right subexpression is printed with the MUL
auxiliary argument.

As first thing, the following op compares the precedence of the expression to be
printed with the precedence of the expected kind of expression: if the former is
lower than the latter, a parenthesized primary expression [ISO 6.5.1] is
printed.
...
(* As discussed in the comments for type TypeName in the C formalization, the
...
In order to do that, we introduce an abstract notion of declarator [ISO 6.7.5],
...
avoids opening a new comment, since comments do not nest in Metaslang) is
...
i.e. we omit "signed" and "int" for signed shorts, ints, longs, and long longs
and we omit "int" for unsigned shorts, ints, longs, and long longs. Printing a
typedef name, a structure type, and the void type is straightforward. *)
...
(* According to the grammar in [ISO 6.7.5], identifier and array declarators are
direct declarators, while pointer declarators are just declarators. So, a
declarator "*x[4]" is parsed as "*(x[4])", i.e. x is an array of pointers. Thus,
we need to parenthesize pointer declarators when they are used inside array
declarators. In this way we minimize the number of parentheses in
declarators. *)
...
"then" branch, optionally followed by the "else" branch. If the (then or else)
branch is a block, we print the opening curly brace at the end of the line where
the if or else appears, then we print the block items indented by one level, and
then we close the curly brace at the beginning of the line. If the then branch
is a block and there is an else branch, the "else" keyword follows the closing
curly brace of the then block, separated by a space. In the op below, the "case
thenBranch ..." expression prints all the needed text up to the "else" keyword,
in order to deal with the various cases.
...
(* Given a constant of type Program defined in explicit syntactic terms (which
...
constraints, the generated program text should be compiled by a compliant C
