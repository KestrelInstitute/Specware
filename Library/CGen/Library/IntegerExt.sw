...
theorem Integer__jacobiSymbol_Obligation_subtype0_aux: 
  "\<lbrakk>a > 0; list_all prime factors; list_all odd factors\<rbrakk> \<Longrightarrow>  
   \<forall>(x::int).  
      int (foldl'
            (\<lambda> ((x_1::nat), (x_2::int)). nat (int x_1 * x_2)) 1
             (map  (\<lambda> (f::Integer__PrimeNat). Integer__legendreSymbol(a, f))  factors))
      = x
   \<longrightarrow>  
   x = - 1 \<or> x = 0 \<or> x = 1"
  apply (induct factors, simp)
  apply (rule allI, rule impI, simp add:  nat_mult_distrib)
  apply (cut_tac a="nat (Integer__legendreSymbol (a, aa))" 
             and l="map (\<lambda>f. Integer__legendreSymbol (a, f)) factors" 
         in foldl_nat_mul_assoc1, simp, 
        rotate_tac -1, thin_tac "?a=?b")
  apply (erule disjE, simp_all add: int_mult split: split_if_asm)
  apply (drule_tac a=a and x=x in Integer__legendreSymbol_cases, auto)
done
end-proof
% ------------------------------------------------------------------------------
...
op jacobiSymbol (a:PosNat, n:PosNat | odd? n)
  ...
  foldl ( * ) 1 (map (fn f: (PrimeNat | odd?) -> legendreSymbol (a, f)) factors)
...
proof Isa jacobiSymbol_Obligation_subtype1
...
% -------------------------------------------------------
proof Isa -verbatim
theorem Integer__isqrt_exists_ge_0:
   "\<exists>i\<Colon>int. 0 \<le> i \<and>  i * i \<le> int n \<and> int n < (i+1)*(i+1)
         \<and> (\<forall>j. j*j \<le> int n \<and> int n < (j+1)*(j+1) \<longrightarrow> j \<le> i)"
  (** Standard proof of intsqrt - unfortunately Isabelle's decision precedures
      are weaker than the ones in Nuprl **)
  apply (induct n) 
  apply (rule_tac x=0 in exI, auto simp add:mult_le_0_iff)
  apply (case_tac "1 + int n <  (i+1)*(i+1)", auto)
  (* case 1 + int n <  (i+1)*(i+1) *)
  apply (rule_tac x=i in exI, auto)
  apply (rule classical, auto simp add: not_le, rotate_tac 5)
  apply (drule zless_imp_add1_zle)
  apply (cut_tac a="i+1" and b=j and c="i+1" and d=j in mult_mono, auto)
  (* case 1 + int n \<ge> (i+1)*(i+1) *)
  apply (rule_tac x="i+1" in exI, auto, simp add: algebra_simps)
  apply (drule zless_imp_add1_zle) 
  apply (cut_tac x="j*j" and y="int n + 1" and z="(i+1)*(i+1)" in order_trans,
         auto)
  apply (rule classical, auto simp add: not_le)
  apply (cut_tac a="i+1" and b=j and c="i+1" and d=j in mult_strict_mono, auto)
done
end-proof
% -------------------------------------------------------
op isqrt (n:Nat) : Nat = maxIn (fn(i:Int) ->
  i * i <= n && (i+1) * (i+1) > n)

proof Isa isqrt_Obligation_subtype
  by (cut_tac Integer__isqrt_exists_ge_0, 
      auto simp add: Integer__hasMax_p_def Integer__isMaxIn_def mem_def)
end-proof

proof Isa isqrt_Obligation_subtype0
 apply (cut_tac Integer__isqrt_exists_ge_0, simp add: Greatest_def GreatestM_def)
...
proof Isa toMinLittleEndian_Obligation_subtype
   apply (simp add: Set_P_def  Set__single_p__stp_def
                    Integer__hasUniqueMinimizer_p__stp_def
                    Integer__minimizers__stp_def mem_def
                    Integer__minimizes_p__stp_def 
   ...
   apply (cut_tac d="hd x" and x=0 and base=base in Integer__littleEndian_p_singleton1,
          simp_all)
   apply (simp add: Integer__littleEndian_p_singleton)
   ...
   apply (rule conjI, rule Integer__toLittleEndian_nonempty,
   ...
   (********)  
   apply (rule_tac base=base and x=x in Integer__toLittleEndian_p_equality, simp_all)
   defer
   ...
   apply (cut_tac x=x and len="ld (x, base)" and base=base in Integer__toLittleEndian_length,
          simp_all add: zpower_int ld_mono ld_positive)
   apply (drule mp, rule Integer__toLittleEndian_nonempty, simp,
          simp add: ld_positive, simp add: zpower_int ld_mono) 
   apply (safe, 
...
proof Isa toMinBigEndian_Obligation_subtype
   ...
   apply (simp add: Set_P_def  Set__single_p__stp_def
                ...
                Integer__littleEndian_p_nil 
                conj_imp)
   apply (thin_tac "\<not>?P", erule exE, rule_tac x="rev xa" in exI, clarsimp)
   apply (simp add: set_eq_iff conj_cong_simp2)
   apply (case_tac "xa=[]", simp_all, clarify)
   apply (drule_tac x="rev xaa" in spec, simp add: rev_swap)
   apply (drule sym, simp, thin_tac "?a=?b", auto simp add: member_def mem_def)
end-proof
...
lemma  Integer__toMinBigEndian_exists:
  ...
  apply (cut_tac base=base and x=x in Integer__toMinBigEndian_Obligation_subtype, simp)
  apply (simp add: Set_P_def mem_def Integer__bigEndian_p_def 
                                     Integer__littleEndian_p_def)
  apply (simp add: Integer__hasUniqueMinimizer_p__stp_def Integer__minimizers__stp_def
                   Integer__minimizes_p__stp_def Set__single_p__stp_def singleton_iff,
         simp add: mem_def)
  apply (erule exE, auto simp add: set_eq_iff)
  apply (drule_tac x=xa in spec, auto)
  apply (simp split: split_if_asm)
  apply (rule_tac x=xa in exI, auto)
  apply (drule_tac x=y in spec, 
         auto simp add: mem_def Integer__bigEndian_p_def Integer__littleEndian_p_def)
done  
