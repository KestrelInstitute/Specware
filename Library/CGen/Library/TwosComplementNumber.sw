...
proof Isa integer_range [simp]
 ...
 apply (case_tac "ys = []", simp_all, 
        frule Bits__toNat_bound, simp only: convert_to_nat zpower_int)
end-proof
...
op minTCNumber (i:Int) : TCNumber =
  minimizer (length, fn x:TCNumber -> toInt x = i)
...
proof Isa minTCNumber_Obligation_subtype
   ...
   apply (rule_tac x="A" in exI, auto simp add: set_eq_iff)
   apply (simp add: TwosComplement__toInt_def mem_def 
                  TwosComplement__nonNegative_p_def fun_Compl_def 
                  TwosComplement__negative_p_def    bool_Compl_def
                  TwosComplement__sign_def)
...
proof Isa length_of_minTCNumber
  ...
  apply (cut_tac i=i in TwosComplement__minTCNumber_Obligation_subtype)
  apply (simp add: Set_P_def mem_def)
  defer
  apply (simp add: Set_P_def mem_def)
  defer
(********** we had this and there must be a generic proof principle *********
  Integer__hasUniqueMinimizer_p__stp ==> LeastM well defined 
*****************************************************************************)
 sorry
end-proof
