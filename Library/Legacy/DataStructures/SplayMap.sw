...
SplayMap qualifying spec {
  ...
  import /Library/Utilities/State
  import SplayTree

  sort Map (key, a) = | EMPTY (key * key -> Comparison)
                      | MAP  {
  ...
  def remove(map,key) = 
         case map
          of EMPTY _ -> map
           | MAP{comp,root,nobj} -> 
        case splay (fn (k,_) -> comp(k,key), State.! root)
          of (Equal, SplayObj{value = value as (_,e), left, right}) -> 
              if nobj = 1
                then EMPTY comp
                else MAP{comp = comp,root = Ref(join(left,right)),nobj = nobj - 1}
  ...
  def listItems map = 
        case map
          of EMPTY _ -> []
           | MAP{root,nobj,comp} -> 
             listItemsf(fn(_,v) -> v,State.! root,[])
  ...
  def listItemsi map =
        case map
          of EMPTY _ -> []
           | MAP{root,nobj,comp} -> 
             listItemsf(fn v -> v,State.! root,[])

  def listDomain map = 
        case map
          of EMPTY _ -> []
           | MAP{root,nobj,comp} -> 
             listItemsf(fn(k,_) -> k,State.! root,[])


  def inDomain(map,key) = 
        case find(map,key) of Some _ -> true | None -> false
  ...
  def foldl abf b map = 
        case map 
          of EMPTY _ -> b
           | MAP{root,comp,nobj} -> 
             foldlAp abf (State.! root,b)

  def compose(map1,map2) = 
        foldri (fn(d,m,map3)->
               case find(map2,m)
                 of Some r -> insert(map3,d,r)
                  | None   -> map3) 
        (empty (case map1
                  of EMPTY comp -> comp
                   | MAP m -> m.comp)) map1 
