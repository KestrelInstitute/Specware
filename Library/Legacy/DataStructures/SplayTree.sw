...
SplayTree qualifying spec {
  ...
  op splay      : fa(a) (a -> General.Comparison) * Splay(a) -> General.Comparison * Splay(a)
  ...
  op adj : fa(a) (a -> General.Comparison) -> Splay(a) -> ans_t(a) * Splay(a) * Splay(a)

  def adj compf sp = 
      case sp
        of SplayNil -> (No,SplayNil,SplayNil)
         | SplayObj{value,left,right} ->
      case compf value 
        of Equal -> (Eq value, left, right)
         | Greater ->
           (case left 
              of SplayNil -> (Gt value,SplayNil,right)
               | SplayObj{value = value_,left = left_,right = right_} ->
                (case compf value_ 
                   of Equal -> (Eq value_,left_,SplayObj{value = value,left = right_,right = right})
                    | Greater ->
                      (case left_ 
                    ...
                    | _ ->
                      (case right_ 
                         of SplayNil -> (Lt value_,left_,SplayObj{value = value,left = right_,right = right})
                          | _ ->
         ...
         | _ (* Less *) -> 
          (case right 
             of SplayNil -> (Lt value,left,SplayNil)
              | SplayObj{value = value_,left = left_,right = right_} ->
              (case compf value_ 
                 of Equal -> (Eq value_,SplayObj{value = value,left = left,right = left_},right_)
                  | Less ->
                   (case right_ 
                      of SplayNil -> (Lt value_,SplayObj{value = value,left = left,right = left_},right_)
                       | _ ->
                         let (V,L,R) = adj compf right_ in
                  ...
                  | _ ->
                   (case left_ 
                      of SplayNil -> (Gt value_,SplayObj{value = value,left = left,right = left_},right_)
                       | _ ->
                        let (V,L,R) = adj compf left_ in
