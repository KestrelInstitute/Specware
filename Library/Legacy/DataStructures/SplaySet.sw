SplaySet qualifying spec {  
  ...
  sort Set key = | EMPTY (key * key -> Comparison)
                 | SET   {
  ...
  op cmp          : fa(key) (key*key -> Comparison) * List(Splay(key)) * List(Splay(key)) -> Comparison
  ...
  def add (set,v) = 
        case set
          of EMPTY comp -> singleton(comp,v)
           | SET{root,nobj,comp} -> 
  ...
  def addList (set,l) = 
        ...
        let (n,r,c) = 
            case set 
              of SET{root,nobj,comp} -> (nobj,State.! root,comp)
  ...
  def delete(set,key) = 
       case set
         of EMPTY _ -> set
          | SET{root,nobj,comp} -> 
            case splay(compf (comp, key), State.! root)
              of (Equal,SplayObj{value,left,right}) -> 
                 if nobj = 1 then empty comp
                  else SET{root = Ref(join(left,right)),nobj = nobj - 1,comp = comp}
               | (_,r) -> (root := r; set)
  ...
  def equal(set1,set2) = 
            case (set1,set2) 
              of (SET{root,nobj,comp},SET{root = r2,nobj = n2,comp = c2}) -> 
  ...
  def next splays =
        case splays
          of ((t as SplayObj vl)::rest) -> (t, left(vl.right, rest))
           | _ -> (SplayNil,[])
  ...
  def cmp(comp,t1,t2) = 
        ...
        case comp(s1.value, s2.value)
          of Equal -> cmp (comp,r1, r2)
           | order -> order
  ...
  def intersection (set1,set2) = 
        case (set1,set2)
          of (EMPTY _,_) -> set1
           | (_,EMPTY _) -> set2
           | (SET{root = r1,nobj = n1,comp = c1},
              SET{root = r2,nobj = n2,comp = c2}) -> 
        case intersectionSplay(c1,State.! r1,State.! r2)
          of (_,0) -> EMPTY c1
  ...
  def count(sp,n) = 
        case sp
          of SplayNil -> n
           | SplayObj {left,right,value} -> 
             count(left,count(right,n + 1))
  ...
  def difference (set1,set2) = 
        case (set1,set2)
          of (EMPTY _,_) -> set1
           ...
           | (SET{root = r1,nobj = n1,comp = c1},
              SET{root = r2,nobj = n2,comp = c2}) -> 
        case diffSplay(c1,State.! r1,State.! r2)
          of (_,0) -> EMPTY c1
  ...
  def filter p set = 
        case set
          of EMPTY _ -> set
           | SET{root,nobj,comp} -> 
        case filterSplay comp (p,State.! root,(0,mkSplayNil))
          of (0,_) -> EMPTY comp
        ...
        case sp 
          of SplayNil -> false
            | SplayObj{value,left,right} ->
                ...
                if existsSplay p left
                then true
                else existsSplay p right 
  def find p set = 
        case set
          of EMPTY _ -> None
           | SET{root,nobj,comp} -> 
             findSplay p (State.! root)
