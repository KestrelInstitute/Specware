;; This is called to start PSL. It is invoked by a command-line
...
(defun build-psl (&optional root-dir)
  ...
  (let* ((root-dir (or root-dir (getenv "SPECWARE4")))
    ...
    (when (file-exists-p world-name)
      (rename-file world-name (concat bin-dir "/PSL-saved.dxl") t))
    (sleep-for 1)
    (simulate-input-expression (format "(excl::dumplisp :name %S)" world-name))
    ))

(defun bootstrap-psl (in-current-dir?)
  (interactive "P")
