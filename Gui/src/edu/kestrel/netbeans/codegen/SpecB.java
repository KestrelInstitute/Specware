 ...
 *
 * $Log$
 * Revision 1.4  2003/02/18 17:59:40  weilyn
...
/**
 * Specialized document binding for a SpecElement.
 *
 */
class SpecB extends Member implements Binding.Spec, TextBinding.Container {
    /**
     * Support for the containment of member elements.
     */
    ContainerSupport    container;
    
    public SpecB(SpecElement el, SourceText s) {
        super(el, s);
    }
     ...
     * not its children, as SpecElement.clone() normally does.
     */
    protected Element cloneElement() {
        SpecElement x = new SpecElement();
        SpecElement my = (SpecElement)getElement();
        try {
    ...
    private SpecElement cloneSpec() {
        return (SpecElement)cloneElement();
    }
    ...
    public void create(PositionBounds bounds) throws SourceException {
        SpecElement c = cloneSpec();
        SpecElement orig = (SpecElement)getElement();
