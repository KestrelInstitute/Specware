...
/** Node representing a proof.
 * @see ProofElement
 */
public class ProofElementNode extends MemberElementNode {
...
/*    private static final String MENU_CREATE_IMPORT;
    ...
    static {
        ResourceBundle bundle = NbBundle.getBundle(ProofElementNode.class);
/*        MENU_CREATE_IMPORT = bundle.getString("MENU_CREATE_IMPORT");
     ...
     * @param element proof element to represent
     * @param children node children
     * @param writeable <code>true</code> to be writable
     */
    public ProofElementNode(ProofElement element, Children children, boolean writeable) {
        super(element, children, writeable);
	String name = element.getName();
        setElementFormat0(sourceOptions.getProofElementFormat());
    }

    /*
      public org.openide.util.HelpCtx getHelpCtx () {
      return new org.openide.util.HelpCtx ("org.openide.src.nodes.ProofNode"); // NOI18N
      }
    */
    ...
    protected ElementFormat getHintElementFormat() {
        return sourceOptions.getProofElementLongFormat();
    }
    ...
    /** Remove this proof from its source file.
     *
    ...
    public void destroy() throws IOException {
		...
		public void run() throws SourceException {
		    ProofElement el = (ProofElement) element;
		    el.getSource().removeProof(el);
		}
	    });
        super.destroy();
    }
	...
	return null;   // new ProofCustomizer((ProofElement)element)
    }
    ...
    private void createMultiPasteTypes(Transferable t, List s, int action) {
        ...
        if (candidates.isEmpty())
            return;
        s.add(new SourceEditSupport.ProofMultiPasteType(this, candidates, (action & NodeTransfer.MOVE) > 0));
    }
    
    // What should this do for proofs???
    private void addNodeCandidate(Collection candidates, Element el) {
        ProofElement enc2 = findEnclosingProof(el);
        SourceElement enc2Src = enc2.getSource();
        String fn2 = enc2.getName();        
        for (Iterator it = candidates.iterator(); it.hasNext(); ) {
            Element can = (Element)it.next();
            ProofElement enc1 = findEnclosingProof(can);
            if (enc1.getSource() != enc2Src) {
                continue;
            }
            // next, if the enclosing proofs are the same...
            if (enc1 == enc2) {
                if (can == enc1) {
                    ...
                    // el == enc2 --> el is declaring proof of `can'
                    // replace `can' with `el'.
                    it.remove();
                    // there can be more such member elements.
                    continue;
                } else {
                    // OK, there is a member of the same proof
                    break;
                }
            }
    ...
    private ProofElement findEnclosingProof(Element el) {
        if (el instanceof ProofElement)
            return (ProofElement)el;
        else if (el instanceof MemberElement) 
	    return findEnclosingProof(((MemberElement)el).getParent());
        else
            return null;
    }
    
    PropertyChangeListener createElementListener() {
	return new ProofElementListener();
    }

    /** Paste element into this proof.
     * @param addingElement Element to add.
     * @param delete Whether element should be deleted from the original proof
    ...
    void pasteElement(final Element addingElement, final boolean delete) throws IOException {
        ...
        SourceEditSupport.invokeAtomicAsUser(element, new SourceEditSupport.ExceptionalRunnable() {
		public void run() throws SourceException {
		    ProofElement proof = (ProofElement) element;
/*		    if (addingElement instanceof ImportElement) {
			proof.addImport((ImportElement)addingElement);
        ...
        if (delete && (!cancelled[0])) {
            final ProofElement origProof;
            SourceElement src = null;

            if (addingElement instanceof MemberElement) {
                origProof = findEnclosingProof(addingElement);
            } else {
                origProof = null;
            }
	    
	    if (src == null && origProof != null) {
		src = origProof.getSource();
	    }

            final SourceElement proofSource = src;
	    SourceEditSupport.ExceptionalRunnable r = new SourceEditSupport.ExceptionalRunnable() {
		    public void run() throws SourceException {
			if (addingElement instanceof MemberElement) {
			    if (origProof != null) {
/*                                if (addingElement instanceof ImportElement) {
				    origProof.removeImport((ImportElement)addingElement);
				} else if (addingElement instanceof SortElement) {
				    ...
				    origSpec.removeClaim((ClaimElement)addingElement);
	                    ...
	                    } else if ((addingElement instanceof ProofElement) &&
				       proofSource != null) {
				proofSource.removeProof((ProofElement)addingElement);
			    }
			}
		    }
		};
    ...
    public NewType[] getNewTypes() {
        if (isWriteable()) {
            return SourceEditSupport.createNewTypes((ProofElement)element);
        } else {
    ...
    public Transferable clipboardCopy() {
        ...
        xt.put(NodeTransfer.createPaste(new SourceEditSupport.PackagePaste(
									   (ProofElement)this.element, false
									   )));
        return xt;
    }
    ...
    public Transferable clipboardCut() {
        ...
        ExTransferable xt = ExTransferable.create(t);
        xt.put(NodeTransfer.createPaste(new SourceEditSupport.PackagePaste(
									   (ProofElement)this.element, true
									   )));
        return xt;
    }
    
    private class ProofElementListener extends ElementListener {
