...
/** Node representing a diagram.
 * @see DiagramElement
 */
public class DiagramElementNode extends MemberElementNode {
...
/*        MENU_CREATE_IMPORT = bundle.getString("MENU_CREATE_IMPORT");
     ...
     * @param element diagram element to represent
     * @param children node children
     * @param writeable <code>true</code> to be writable
     */
    public DiagramElementNode(DiagramElement element, Children children, boolean writeable) {
        super(element, children, writeable);
	String name = element.getName();
        setElementFormat0(sourceOptions.getDiagramElementFormat());
    }

    /*
      public org.openide.util.HelpCtx getHelpCtx () {
      return new org.openide.util.HelpCtx ("org.openide.src.nodes.DiagramNode"); // NOI18N
      }
    */
    ...
    protected ElementFormat getHintElementFormat() {
        return sourceOptions.getDiagramElementLongFormat();
    }
    ...
    /** Remove this diagram from its source file.
     *
	...
	return null;   // new DiagramCustomizer((DiagramElement)element)
    }
    ...
    private void createMultiPasteTypes(Transferable t, List s, int action) {
        ...
        if (candidates.isEmpty())
            return;
        s.add(new SourceEditSupport.DiagramMultiPasteType(this, candidates, (action & NodeTransfer.MOVE) > 0));
    }
    
    // What should this do for diagrams???
    private void addNodeCandidate(Collection candidates, Element el) {
        DiagramElement enc2 = findEnclosingDiagram(el);
        SourceElement enc2Src = enc2.getSource();
        String fn2 = enc2.getName();        
        for (Iterator it = candidates.iterator(); it.hasNext(); ) {
            Element can = (Element)it.next();
            DiagramElement enc1 = findEnclosingDiagram(can);
            if (enc1.getSource() != enc2Src) {
                continue;
            }
            // next, if the enclosing diagrams are the same...
            if (enc1 == enc2) {
                if (can == enc1) {
                    ...
                    // el == enc2 --> el is declaring diagram of `can'
                    // replace `can' with `el'.
                    it.remove();
                    // there can be more such member elements.
                    continue;
                } else {
                    // OK, there is a member of the same diagram
                    break;
                }
            }
    ...
    private DiagramElement findEnclosingDiagram(Element el) {
        if (el instanceof DiagramElement)
            return (DiagramElement)el;
        else if (el instanceof MemberElement) 
	    return findEnclosingDiagram(((MemberElement)el).getParent());
        else
            return null;
    }
    
    PropertyChangeListener createElementListener() {
	return new DiagramElementListener();
    }

    /** Paste element into this diagram.
     * @param addingElement Element to add.
     * @param delete Whether element should be deleted from the original diagram
...
/*		    if (addingElement instanceof ImportElement) {
        ...
        if (delete && (!cancelled[0])) {
            final DiagramElement origDiagram;
            SourceElement src = null;

            if (addingElement instanceof MemberElement) {
                origDiagram = findEnclosingDiagram(addingElement);
            } else {
                origDiagram = null;
            }
	    
	    if (src == null && origDiagram != null) {
		src = origDiagram.getSource();
	    }

            final SourceElement diagramSource = src;
...
/*                                if (addingElement instanceof ImportElement) {
				    ...
				    origSpec.removeClaim((ClaimElement)addingElement);
	                    ...
	                    } else if ((addingElement instanceof DiagramElement) &&
				       diagramSource != null) {
				diagramSource.removeDiagram((DiagramElement)addingElement);
			    }
			}
		    }
		};
    ...
    public NewType[] getNewTypes() {
        if (isWriteable()) {
            return SourceEditSupport.createNewTypes((DiagramElement)element);
        } else {
    ...
    public Transferable clipboardCopy() {
        ...
        xt.put(NodeTransfer.createPaste(new SourceEditSupport.PackagePaste(
									   (DiagramElement)this.element, false
									   )));
        return xt;
    }
    ...
    public Transferable clipboardCut() {
        ...
        ExTransferable xt = ExTransferable.create(t);
        xt.put(NodeTransfer.createPaste(new SourceEditSupport.PackagePaste(
									   (DiagramElement)this.element, true
									   )));
        return xt;
    }
    
    private class DiagramElementListener extends ElementListener {
