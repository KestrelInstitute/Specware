...
%However, since a well-formed spec has no type variable declarations, an op
%declaration in a well-formed spec automatically satisfies the restriction.
\item
...
%As in other cases, all type variables in $\expr$ are automatically in $\tvarS$
%in well-formed specs, but that is not required in well-formed contexts in
%general.
\item
...
premises $\istyO{\typ_1}$ and $\istyO{\typ_2}$, then \thmref{thm-exty-wf}
