...
<<<<<<< Substitute.sw
      | (PSpec pSpec, Morph morph) ->
           let def appSub x y = unEnv (fn (x,y) -> applySubstitution x y) (x,y) in
           let timeStamp = max (spec_timestamp, sm_timestamp) in
           let dep_URIs  = listUnion (spec_dep_URIs, sm_dep_URIs) in {
             dyCtxt <- dynamicSpec pSpec;
             print "\nApplying morphism to dynamic spec\n";
             newDyCtxt <- attemptSubstitution dyCtxt morph term_pos;
...
%            let dep_URIs  = listUnion (spec_dep_URIs, sm_dep_URIs) in {
%              dyCtxt <- dynamicSpec pSpec;
%              newDyCtxt <- attemptSubstitution dyCtxt sm sm_tm term_pos;
%              procs <- procedures pSpec
...
<<<<<<< Substitute.sw
  PolyMap.foldMapM
  op foldMapM : fa(Dom,Cod,a) (a -> Dom -> Cod -> Monad a) -> a -> Map (Dom,Cod) -> Monad a
  def foldMapM 
   ...
   foldMapM f unit map =
     case map of
       | [] -> unit
       | (dom,cod)::rest ->  {
           newUnit <- f unit dom cod
           foldMapM f newUnit rest
             return (PSpec new_spec, timeStamp, dep_URIs)
           }
