...
\begin{spec}
  def SpecCalc.evaluateSpecMorph (domTerm,codTerm,morphRules) = {
    ...
    case (domValue,codValue) of
      ...
      | (PSpec pSpec, Spec spc) -> {
          morph <- makeSpecMorphism(pSpec.dynamicSpec, spc, morphRules);
          return (Morph morph,max(domTimeStamp,codTimeStamp),
		   listUnion(domDepURIs,codDepURIs))}
      | (PSpec pSpec1, PSpec pSpec2) -> {
          morph <- makeSpecMorphism(pSpec1.dynamicSpec, pSpec2.dynamicSpec, morphRules);
          return (Morph morph,max(domTimeStamp,codTimeStamp),
		   listUnion(domDepURIs,codDepURIs))}
      | (Spec _, _) -> raise
