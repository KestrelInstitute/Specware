...
def conformOpDecls(spc) =
  ...
  let ops = foldriAQualifierMap
            (fn(q,id,opinfo as (opnames,fixity,(tyvars,srt),opterms),newops) ->
	     let newopterms = List.map (fn(tyvars,term) -> (tyvars,conformOpDeclsTerm(spc,srt,term,id))) opterms in
	     let newopinfo = (opnames,fixity,(tyvars,srt),newopterms) in
	     insertAQualifierMap(newops,q,id,newopinfo))
	    emptyAQualifierMap spc.ops
  in
...
def conformOpDeclsTerm(spc,srt,term,_) =
 ...
 def getFoldedDomain(spc,qid) =
   case findTheOp(spc,qid) of
     | None -> None
     | Some(opnames,fixity,(tyvars,srt),opterms) ->
       let srt = unfoldToArrow(spc,srt) in
       case srt of
	 | Arrow(domsrt as (Base _),ransrt,_) ->
	 let usrt = unfoldToProduct(spc,domsrt) in
...
def addEqOpsToSpec(spc) =
  let res = foldriAQualifierMap
             (fn(qid,name,sortinfo,spc) ->
	      let spc = addEqOpsFromSort(spc,Qualified(qid,name),sortinfo) in
	      spc
	     ) spc spc.sorts
  in
  res
...
def addEqOpsFromSort(spc,qid,(sortnames,tyvars0,sortschemes)) =
  let
  ...
  let
    def addEqOp(eqqid as Qualified(eq,eid),osrt,body,b) =
      ...
      let sortscheme = ([],getEqOpSort(osrt,b)) in
      let opinfo = ([eqqid],Nonfix:Fixity,sortscheme,[([],term)]) in
      let ops = insertAQualifierMap(spc.ops,eq,eid,opinfo) in
      setOps(spc,ops)
  in
  case sortschemes of
    | [] -> spc
	...
	in
	  ...
	  | _ ->
		...
		foldr (fn((fid,optfsrt),match) ->
		       ...
		       let ypat = EmbedPat(fid,case optfsrt of None -> None | Some fsrt -> Some(VarPat(("y0",fsrt),b)),osrt,b) in
		       let eqFsrt =
		         let def bcase(fsrt) =
			     let eqsrt = getEqOpSort(fsrt,b) in
			     Apply(Fun(Equals,eqsrt,b),Record([("1",Var(("x0",fsrt),b)),("2",Var(("y0",fsrt),b))],b),b)
			 in
		         case optfsrt of
			   | None -> mkTrue()
			   ...
			   | Some fsrt ->
			     ...
			     case fsrt of
			       | Product(fields,_) -> getEqTermFromProductFields(fields,fsrt,Var(("x0",fsrt),b),Var(("y0",fsrt),b))
			       | _ -> bcase(fsrt)
		       in
