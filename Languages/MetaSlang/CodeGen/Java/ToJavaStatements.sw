...
op termToExpression: TCx * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateTermsToExpressions: TCx * List Term * Nat * Spec -> Block * List Java.Expr * Nat
op translateApplyToExpr: TCx * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateRecordToExpr: TCx * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateIfThenElseToExpr: TCx * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateLetToExpr: TCx * Term * Nat * Spec -> Block * Java.Expr * Nat

def translateApplyToExpr(tcx, term as Apply (opTerm, argsTerm, _), k, spc) =
  case opTerm of
    | Fun (Equals , srt, _) -> translateEqualsToExpr(tcx, argsTerm, k, spc)
    | Fun (Project (id) , srt, _) -> translateProjectToExpr(tcx, id, argsTerm, k, spc)
    | Fun (Embed (id, _) , srt, _) -> translateConstructToExpr(tcx, srtId(termSort(term)), id, argsTerm, k, spc)
    ...
    if all (fn (srt) -> baseType?(srt)) dom
      then
	if baseType?(rng)
	  then translateBaseApplToExpr(tcx, id, argsTerm, k, spc)
	else translateBaseArgsApplToExpr(tcx, id, argsTerm, rng, k, spc)
    else
      translateUserApplToExpr(tcx, id, dom, argsTerm, k, spc)

op translateEqualsToExpr: TCx * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateProjectToExpr: TCx * Id * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateConstructToExpr: TCx * Id * Id * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateBaseApplToExpr: TCx * Id * Term * Nat * Spec -> Block * Java.Expr * Nat
op translateBaseArgsApplToExpr: TCx * Id * Term * Type * Nat * Spec -> Block * Java.Expr * Nat
op translateUserApplToExpr: TCx * Id * List Type * Term * Nat * Spec -> Block * Java.Expr * Nat

def translateEqualsToExpr(tcx, argsTerm, k, spc) =
  let args = applyArgsToTerms(argsTerm) in
  let (newBlock, [jE1, jE2], newK) = translateTermsToExpressions(tcx, args, k, spc) in
  (newBlock, mkJavaEq(jE1, jE2, srtId(termSort(hd(args)))), newK)

def translateProjectToExpr(tcx, id, argsTerm, k, spc) =
  let args = applyArgsToTerms(argsTerm) in
  let (newBlock, [e], newK) = translateTermsToExpressions(tcx, args, k, spc) in
  (newBlock, mkFldAcc(e, id), newK)

def translateConstructToExpr(tcx, srtId, opId, argsTerm, k, spc) =
  let args = applyArgsToTerms(argsTerm) in
  let (newBlock, javaArgs, newK) = translateTermsToExpressions(tcx, args, k, spc) in
  (newBlock, mkMethInv(srtId, opId, javaArgs), newK)
  
def translateBaseApplToExpr(tcx, opId, argsTerm, k, spc) =
  let args = applyArgsToTerms(argsTerm) in
  let (newBlock, javaArgs, newK) = translateTermsToExpressions(tcx, args, k, spc) in
  if javaBaseOp?(opId)
    then 
	...
	then (newBlock, mkBinExp(opId, javaArgs), newK)
      else (newBlock, mkUnExp(opId, javaArgs), newK)
  else (newBlock, mkMethInv("Primitive", opId, javaArgs), newK)

def translateBaseArgsApplToExpr(tcx, opId, argsTerm, rng, k, spc) =
  let args = applyArgsToTerms(argsTerm) in
  let (newBlock, javaArgs, newK) = translateTermsToExpressions(tcx, args, k, spc) in
  if javaBaseOp?(opId)
    then (newBlock, mkBinExp(opId, javaArgs), newK)
  else (newBlock, mkMethInv(srtId(rng), opId, javaArgs), newK)

def translateUserApplToExpr(tcx, opId, dom, argsTerm, k, spc) =
  let args = applyArgsToTerms(argsTerm) in
  let Some(h, _) = findIndex (fn(srt) -> userType?(srt)) dom in
  let (newBlock, javaArgs, newK) = translateTermsToExpressions(tcx, args, k, spc) in
  let topJArg = nth(javaArgs, h) in
  let resJArgs = deleteNth(h, javaArgs) in
  (newBlock, mkMethExprInv(topJArg, opId, resJArgs), newK)

def translateRecordToExpr(tcx, term as Record (fields, _), k, spc) =
  let recordTerms = recordFieldsToTerms(fields) in
  let recordSrt = termSort(term) in
  let (newBlock, javaArgs, newK) = translateTermsToExpressions(tcx, recordTerms, k, spc) in
  let srts = sortsAsList(spc) in
  let foundSrt = find (fn (qualifier, id, (_, _, [(_,srt)])) -> equalSort?(recordSrt, srt)) srts in
  case foundSrt of
     | Some (q, recordClassId, _) ->  (newBlock, mkNewClasInst(recordClassId, javaArgs), newK)
     | None -> fail("Could not find record sort.")
  %%Fix here HACK!!!


def translateIfThenElseToExpr(tcx, term as IfThenElse(t0, t1, t2, _), k, spc) =
  let (b0, jT0, k0) = termToExpression(tcx, t0, k+1, spc) in
  let (b1, jT1, k1) = termToExpression(tcx, t1, k0, spc) in  
  let (b2, jT2, k2) = termToExpression(tcx, t2, k1, spc) in  
  case b1++b2 of
    | [] ->
...
def translateLetToExpr(tcx, term as Let (letBindings, letBody, _), k, spc) =
  let [(VarPat (v, _), letTerm)] = letBindings in
  let (b0, jLetTerm, k0) = termToExpression(tcx, letTerm, k, spc) in
  let (b1, jLetBody, k1) = termToExpression(tcx, letBody, k0, spc) in
  let (vId, vSrt) = v in
  let vInit = mkVarInit(vId, srtId(vSrt), jLetTerm) in
  (b0++[vInit]++b1, jLetBody, k1)

def translateTermsToExpressions(tcx, terms, k, spc) =
    case terms of
    | [] -> ([], [], k)
    | term::terms ->
    let (newBody, jTerm, newK) = termToExpression(tcx, term, k, spc) in
    let (restBody, restJTerms, restK) = translateTermsToExpressions(tcx, terms, newK, spc) in
    (newBody++restBody, cons(jTerm, restJTerms), restK)

  
def termToExpression(tcx, term, k, spc) =
  case term of
    | Var ((id, srt), _) ->
    ...
    | Fun (Op (Qualified (q, id), _), srt, _) -> 
       if baseType?(srt) 
	 then (mts, mkQualJavaExpr("Primitive", id), k)
       else
	 let Base (Qualified (q, srtId), _, _) = srt in
	 (mts, mkQualJavaExpr(srtId, id), k)
    | Fun (Nat (n),_,__) -> (mts, mkJavaNumber(n), k)
    | Fun (Embed (c, _), srt, _) -> (mts, mkMethInv(srtId(srt), c, []), k)
    | Apply (opTerm, argsTerm, _) -> translateApplyToExpr(tcx, term, k, spc)
    | Record _ -> translateRecordToExpr(tcx, term, k, spc)
    | IfThenElse _ -> translateIfThenElseToExpr(tcx, term, k, spc)
    | Let _ -> translateLetToExpr(tcx, term, k, spc)
