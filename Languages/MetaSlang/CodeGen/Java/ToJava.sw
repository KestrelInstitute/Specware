...
def specToJava(basespc,spc,optspec,filename) =
 ...
 op SpecTransform.showSpec : Spec -> String -> ()
 op SpecTransform.showSpecFlat : Spec -> String -> ()
 op CG.removeUnusedOps : QualifiedIds -> QualifiedIds -> Spec -> Spec

 op SpecTransform.MyOut (spc : Spec) : Spec =
  let base = getBaseSpec() in

    let _ = SpecTransform.showSpecFlat spc "111 -- start" in
    let _ = writeLine "========" in

  let spc = substBaseSpecs   spc in

    let _ = SpecTransform.showSpecFlat spc "222 -- substBaseSpecs" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "333 -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = substBaseSpecsJ spc in

    let _ = SpecTransform.showSpecFlat spc "444 -- substBaseSpecsJ" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "444 B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = normalizeTopLevelLambdas spc in

    let _ = SpecTransform.showSpecFlat spc "555 -- normalizeTopLevelLambdas " in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "555 B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = instantiateHOFns spc in

    let _ = SpecTransform.showSpecFlat spc "666 -- instantiateHOFns " in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "666 B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = subtractSpec spc base in

    let _ = SpecTransform.showSpecFlat spc "777 -- subtractSpec" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "777 B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = addMissingFromBase(base,spc,builtinTypeOp) in

    let _ = SpecTransform.showSpecFlat spc "888 -- addMissingFromBase" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "888 B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = substBaseSpecsJ spc in

    let _ = SpecTransform.showSpecFlat spc "999 -- substBaseSpecsJ" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "999 B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = lambdaLiftWithImportsSimulatingClosures spc in

    let _ = SpecTransform.showSpecFlat spc "AAAA -- lambdaLiftWithImportsSimulatingClosures" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "AAAA B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = unfoldTypeAliases spc in

    let _ = SpecTransform.showSpecFlat spc "BBBB -- unfoldTypeAliases" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "BBBB B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = expandRecordMerges spc in

    let _ = SpecTransform.showSpecFlat spc "CCCC -- expandRecordMerges" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "CCCC B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = identifyIntTypes spc in

    let _ = SpecTransform.showSpecFlat spc "DDDD -- identifyIntTypes" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "DDDD B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = poly2mono(spc,false) in % false means we do not keep declarations for polymorphic types and ops in the new spec

    let _ = SpecTransform.showSpecFlat spc "EEEE -- poly2mono" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "EEEE B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = letWildPatToSeq spc in

    let _ = SpecTransform.showSpecFlat spc "FFFF -- letWildPatToSeq" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "FFFF B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = translateMatchJava spc in

    let _ = SpecTransform.showSpecFlat spc "GGGG -- translateMatchJava" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "GGGG B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = simplifySpec spc in

    let _ = SpecTransform.showSpecFlat spc "HHHH -- simplifySpec" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "HHHH B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = etaExpandDefs spc in

    let _ = SpecTransform.showSpecFlat spc "IIII -- etaExpandDefs" in
    let _ = writeLine "========" in

  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "IIII B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let spc = distinctVariable(spc) in

    let _ = SpecTransform.showSpecFlat spc "JJJJ -- distinctVariable" in
    let _ = writeLine "========" in


  let spc = removeUnusedOps [mkQualifiedId("my","solve")] [] spc in %  (3) do early to minimize wasted motion 

    let _ = SpecTransform.showSpecFlat spc "JJJJ B -- removeUnusedOps" in
    let _ = writeLine "========" in

  let jspc = generateJavaCodeFromTransformedSpec spc in
  let jfiles = processOptions(jspc,None,"aaa.bbb") in
  let _ = List.app printJavaFile jfiles in
  spc
