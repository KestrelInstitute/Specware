...
%    in
  ...
  def sort2type (ctxt, spc, tvs, srt) =
    ...
    case usrt of
     ...
     %	   "(e.g. \"{l:List("^printSort(psrt)^")| length(l) <= 32}\")")
      ...
      | Base(Qualified (q, id),_,_) -> Base (q, id)
  ...
  def getOpDefinition (spc, (Qualified (q, id))) =
...
%    in
  ...
  def getBuiltinExpr(ctxt,spc,term,args) =
    ...
    case (term,args) of
      ...
      | (Fun(Op(Qualified("Nat","succ"),_),_,_),[t1])
        -> let e1 = t2e t1 in
 ...
 def foldSort (spc, srt) =
   let opt_srt =
	  ...
	  case opt_srt of
		...
		if equivTypes? spc (srt, srt0) then
