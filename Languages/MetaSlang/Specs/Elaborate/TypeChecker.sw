...
spec { 
  ...
  def selectTermWithConsistentSort (env, id, pos, terms, srt) =
   ...
   case filter (consistentSortOp? (env, withAnnS (rsort, srtPos))) terms of
    | []            -> (error (env, 
			...
			None)
    | [term]        -> Some term
    | first_tm :: _ -> if env.firstPass?
			 then None
		       else %% Some first_tm
			 (error (env, 
				 "Several matches for overloaded op "^id^" of sort "^ printSort srt^
				 (foldl (fn (tm, str) -> str ^ 
					 (case tm of
			                   | Fun (OneName  (     id2, _), _, _) -> str^" "^id2
					   | Fun (TwoNames (id1, id2, _), _, _) -> str^" "^id1^"."^id2))
				  " : "
				  terms),
				 pos);
			  None)
  ...
  let
	def elab (p:PPattern):PPattern * LocalEnv =
	    case p
	      of WildPat (s, pos) -> (WildPat (elaborateSort (env, s, sort1), pos), env)
	       | BoolPat _ -> 
		(elaborateSort (env, sort1, type_bool); (p, env))
	       | NatPat _ ->  
		(elaborateSort (env, sort1, type_nat); (p, env))
	       | StringPat _ ->  
	    	(elaborateSort (env, sort1, type_string);  (p, env))
	       | CharPat _ ->  
	        (elaborateSort (env, sort1, type_char); (p, env))
	       | VarPat ((id, srt), pos) -> 
	        let srt = elaborateSort (env, srt, sort1)  in 
		(case lookupEmbedId (env, id, srt)
		  of Some None -> 
		     (EmbedPat (id, None, srt, pos), env)
		   | Some _ -> 
		   ...
		   | None ->
		     if undeterminedSort? srt
		       then case StringMap.find (env.constrs, id)
			      of None ->
		       ...
		       else (VarPat ((id, srt), pos), addVariable (env, id, srt)))
	      | SortedPat (pat, srt, _) -> 
		let srt = elaborateSort (env, srt, sort1) in
		...
		let sort0 = if undeterminedSort? sort0
		             then  % See if there is only one constructor with this name
			       case StringMap.find (env.constrs, embedId)
				 of Some [srt_info] ->
				    let (_, c_srt) = copySort srt_info in
				    elaborateSort (env, c_srt, sort1)
				  | _ -> sort0
			     else sort0 in
		if env.firstPass? & undeterminedSort? sort0
		  then (EmbedPat (embedId, pattern, sort0, pos), env)
		else
		let srt = lookupEmbedId (env, embedId, sort0) in
		let (env, pat):LocalEnv * Option (PPattern) = 
		    case (srt, pattern)
		      of (Some (Some srt), Some pat) -> 
			 let (pat, env) = elaboratePattern (env, pat, srt)
			 in
			     (env, Some pat)
			 
		       | (Some None, None) -> (env, None)
		       | (None, None) -> 
			 (error (env, "Sort for constructor "^embedId^" not found. Resolving with sort "^printSort sort1, pos);
			  (env, None))
		       | (None, Some pat) -> 
			 let alpha = freshMetaTyVar pos in
			 ...
			 (error (env, "Sort for constructor "^embedId^" not found. Resolving with sort "^printSort sort1, pos);
			     (env, None))
		       | (Some None, Some pat) -> 
			 (error (env, newLines ["Constructor "^embedId^" takes no argument", 
						"but was given "^printPattern pat], 
				 pos);
			  (env, Some pat))
		       | (Some (Some _), None) -> 
			 (error (env, "Argument expected for constructor "^embedId, pos);
			  (env, None))
	    in
