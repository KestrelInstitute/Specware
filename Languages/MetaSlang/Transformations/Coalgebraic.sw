...
op doIntroduceRewriting?: Bool = true
  ...
  {info <- findTheOp spc intro_qid;
   ...
   let def addToDef(info, spc) =
         ...
         case range_*(spc, ty) of
           | Subsort(result_ty, Lambda([(_, _, _)], _), _)  | equalTypeSubtype?(result_ty, state_ty, true) ->
             ...
             (case extractCoalgebraicComps(ty, result_tm, state_ty, spc) of
               ...
               | Some (params, result_var, thms) ->
                 ...
                 let rhs1 = if doIntroduceRewriting? then rewrite(toPathTerm raw_rhs, context, qid, rules, 2) else raw_rhs in
                 % let  _ = writeLine(" --->\n"^printTerm rhs1) in
                 let rules = makeRules (context, spc, [Fold intro_qid]) ++ rules in
                 let opt_rhs = if doIntroduceRewriting? then rewrite(toPathTerm rhs1, context, qid, rules, maxRewrites) else rhs1 in
