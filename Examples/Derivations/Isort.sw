...
(****  How to run this metaprogram and then execute the generated code  ***

There are two approaches.  

1. The simplest approach is to use Specware command-line tools:

* ctext <path>/Isort.sw         %% evaluate this metaprogram (where <path> 
                                %% is the path to this file)

* ld isort.lisp                 %% load the lisp code (or use cl to compile-and-load)

* el Isort [10, 4, 7, 3, 12, 1] %% execute the code

2. Alternately, simply evaluate the file (e.g. c-c p) and then use
SBCL Commonlisp commands to load/compile the generated file and run it
in the *Spaceware Shell* or any other Lisp environment.
...
asInsertionSort = morphism ../../Library/AlgorithmTheories/DivideAndConquer#DC_01 -> Sorting_as_DC
            { D +-> coBagNat
            , R +-> ListNat
            , O +-> sorted2?
            , E    +-> Nat
...
InsertionSort = ../../Library/AlgorithmTheories/DivideAndConquer#DC_01_scheme[asInsertionSort]
...
Here we manually introduce code to implement the two composition
 operators whose specification we calculated above. The insert
 operator can itself be synthesized using the D&C strategy.  
*)
