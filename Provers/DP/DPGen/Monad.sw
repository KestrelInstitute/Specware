...
spec
  ...
  import StateAndExceptionMonads, State
  ...
  op Monad.getInfo: Ineq -> M Info
  def Monad.getInfo (i) =
    fn state ->
    (RETURN(State.getInfo i state), state)

  op Monad.putInfo: Ineq * Info -> M ()
  def Monad.putInfo (i, info) =
    fn state ->
    (RETURN(()), (State.putInfo(i, info) state))
