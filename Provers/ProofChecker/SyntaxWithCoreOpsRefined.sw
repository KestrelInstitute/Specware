spec
  ...
  (* This spec refines some of the non-executable ops in spec
  `SyntaxWithCoreOps', making them executable. Only the ops that are needed to
  execute op `check' in spec `Check' are refined. *)

  import Syntax, Positions

  op conjoinAll : Expressions -> Expression
  def conjoinAll eS =  % executable
    if eS = empty then TRUE
    else if single? eS then theElement eS
    else first eS &&& conjoinAll (rtail eS)

  op disjoinAll : Expressions -> Expression
  def disjoinAll eS =  % executable
    if eS = empty then TRUE
    else if single? eS then theElement eS
    else first eS ||| disjoinAll (rtail eS)

  % copied from spec `SyntaxWithCoreOps':
  ...
  def contextDefinesType?(cx,tn) =  % executable
    exists? (fn | typeDefinition (mustBe_tn, _, _) -> mustBe_tn = tn
                | _ -> false) cx

  op contextDefinesOp? : Context * Operation -> Boolean
  def contextDefinesOp?(cx,o) =  % executable
    exists? (fn | opDefinition (mustBe_o, _, _) -> mustBe_o = o
                | _ -> false) cx
  ...
  def exprSubstOK?(e,sbs) =  % executable
    forall? (fn v -> exprFreeVars (sbs @ v) /\ captVars v e = empty)
            (domain sbs)
  ...
  % copied from spec `SyntaxWithCoreOps' (no need to make them executable):
  ...
  % copied from spec `SyntaxWithCoreOps' (no need to make it executable):
  ...
  % as in spec `SyntaxWithCoreOps' but with `assert' removed:
    ...
    if pos = empty then empty
         ...
         case e of
           ...
           | nonRecursiveLet(p,e,e1) -> if i = 0
                                        then captVarsAt(pos,e)
                                        else pattVars p \/ captVarsAt(pos,e1)
