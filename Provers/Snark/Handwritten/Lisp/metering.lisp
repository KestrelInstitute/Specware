...
(eval-when (compile load eval)
  #+(and :cmu :new-compiler)
  (deftype time-type () 'unsigned-byte)		;was (unsigned-byte 29) - MES
  #+(and :cmu :new-compiler)
  (deftype consing-type () 'unsigned-byte)	;was (unsigned-byte 29) - MES
  #-(and :cmu :new-compiler)
...
#+(and :allegro (not :allegro-v3.1))
(defvar *GC-SPACE-ARRAY* (make-array 4 :element-type '(unsigned-byte 32)))
#+(and :allegro (not :allegro-v3.1))
(defun BYTES-CONSED ()
  (system:gsgc-totalloc *gc-space-array* t)
  (+
    (* 8 (aref *gc-space-array* 0))		;conses
    (* 24 (aref *gc-space-array* 1))		;symbols
    (aref *gc-space-array* 2)			;other bytes
    ))
...
#+:allegro
(defmacro GET-CONS ()
  `(the consing-type (bytes-consed)))

;;; Macintosh Allegro Common Lisp 1.3.2
;;; Based on CCL's sample code for memory usage.
;;; The key trick here is that we maintain the information about total
...
#-(or :cmu CLISP :lcl3.0 :lcl4.0 :mcl1.3.2 :mcl :excl :Genera) 
(progn
 (eval-when (compile eval)
   (warn
...
(proclaim '(integer *total-calls*))	;was fixnum - MES
    ...
    `(lambda (name)
       (let ((inclusive-time 0)
         ...
         (declare (type time-type inclusive-time)
                  ...
                  (integer calls)		;was fixnum - MES
                  (integer nested-calls))	;was fixnum - MES
         (pushnew name *monitored-functions*)

         (setf (place-function name)
               #'(lambda (,@required-args
                          ,@(when optionals-p `(&rest optional-args)))
                   ,(when optionals-p `(declare #+:ANSI-CL (dynamic-extent optional-args)))   ;;; &rest optional-args can be stack allocated
...
;                        (old-nested-calls nested-calls)
                     ...
                     (declare (type time-type prev-total-time)
                              ...
                              (integer prev-total-calls))	;was fixnum - MES
                     (multiple-value-prog1
                       ...
                       (let ((delta-time (- (get-time) start-time))
                         ...
                         (incf nested-calls (the integer	;was fixnum - MES
                                                 (- *total-calls*
                                                    prev-total-calls)))
...
(defvar *DEFAULT-THRESHOLD* 0.002)	;was 0.005 - MES

(defmacro MONITOR-FORM (form 
                        &optional (nested :exclusive) (threshold *default-threshold*)
                        (key :percent-time))
...
;;; ********************************
(defconstant OVERHEAD-ITERATIONS 100000
...
of an empty function many times." 
  (let ((previous-monitor-time-overhead *monitor-time-overhead*)
	(previous-monitor-cons-overhead *monitor-cons-overhead*))
...
;    (stub-function nil))
    ...
    (format t "~%Monitor time overhead/call = ~7,3,3F" (/ *monitor-time-overhead* time-units-per-second))
    (when previous-monitor-time-overhead
      (format t "  (previously ~7,3,3F)" (/ previous-monitor-time-overhead time-units-per-second)))
    (format t "~%Monitor cons overhead/call = ~7,3F" *monitor-cons-overhead*)
    (when previous-monitor-cons-overhead
      (format t "  (previously ~7,3F)" previous-monitor-cons-overhead))))
...
of an empty function many times." 
  (let ((previous-monitor-time-overhead *monitor-time-overhead*)
	...
	#+:Genera (declare (ignore x))
	(funcall overhead-function overhead-function))
      (setq time (- (get-time) start-time))
      (setq cons (- (get-cons) start-cons)))
    (monitor stub-function)
    (reset-all-monitoring)
    (let ((overhead-function (symbol-function 'stub-function))
	  (start-time (get-time))
	...
	#+:Genera (declare (ignore x))
	(funcall overhead-function overhead-function))
      (setq time (- (get-time) start-time time))
      (setq cons (- (get-cons) start-cons cons)))
    (unmonitor stub-function)
    (setq *monitor-time-overhead* (/ (float time) overhead-iterations))
    (setq *monitor-cons-overhead* (/ (float cons) overhead-iterations))
    (format t "~%Monitor time overhead/call = ~7,3,3F" (/ *monitor-time-overhead* time-units-per-second))
    (when previous-monitor-time-overhead
      (format t "  (previously ~7,3,3F)" (/ previous-monitor-time-overhead time-units-per-second)))
    (format t "~%Monitor cons overhead/call = ~7,3F" *monitor-cons-overhead*)
...
below THRESHOLD % of the time will not be reported."
  ...
  (let ((total-time 0)
    ...
    (setq total-cons (float total-cons))
    ...
    (if (zerop total-time)
		...
		((zerop calls)
		 (push (if (symbolp name) 
		       ...
		       *no-calls*))
		(t
		 (setq time (max 0.0 (float time)))
		 (setq cons (max 0.0 (float cons)))
		 (push (make-monitoring-info
                         (format nil "~S" name) ; name
                         calls			; calls
                         (/ time time-units-per-second) ; time in secs
                         (round cons)		; consing
                         (/ time total-time)	; percent-time
                         (if (zerop total-cons) 0.0 (/ cons total-cons))	; percent-cons
                         (/ (/ time calls)	; time-per-call
                            time-units-per-second)	; sec/call
                         (round (/ cons calls)))	; cons-per-call
		       *monitor-results*)))))
          (display-monitoring-results threshold key ignore-no-calls)))))

(defun DISPLAY-MONITORING-RESULTS (&optional (threshold *default-threshold*) (key :percent-time)
                                             (ignore-no-calls t))
  (let ((max-length 8)                  ; Function header size
    ...
    (incf max-length 2)
    (format *trace-output*
            "~&        ~VT   %     %                uSec/ Bytes/     Total     Total~
             ~&Function~VTTime Bytes      Calls      Call   Call      Time     Bytes~
             ~&~V,,,'-A"
            max-length max-length
            (+ max-length 58) "-")      
    (dolist (result *monitor-results*)
      (incf total-all-calls (m-info-calls result))
      (when (or (zerop threshold)
                (> (m-info-percent-time result) threshold))
        (format *trace-output* 
                "~&~A:~VT~4,1,2F~6,1,2F ~10D ~9D ~6D ~9,3F ~9D"
                (m-info-name result)
                max-length
                (m-info-percent-time result)
                (m-info-percent-cons result)
                (m-info-calls result)
                (round (* 1000000 (m-info-time-per-call result)))
...
#+ignore
    (format *trace-output* 
            "~&~V,,,'-A~
            ~&TOTAL:~VT~5,1,2F~6,1,2F ~7D                  ~9,5F ~8D~
            ~&Estimated monitoring overhead: ~4,2F seconds for listed functions~
            , ~4,2F seconds total"
            (+ max-length 54) "-"
            (1- max-length)
	    ...
	    (/ (* *monitor-time-overhead* total-all-calls)
	       time-units-per-second))
