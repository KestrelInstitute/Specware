...
(defun manual-ordering-compare-terms (x y subst)
  (setq x (renumber x subst))
  (setq y (renumber y subst))
  (let (v)
    (cond
	 ...
	 (terpri) (print-term x)
	 (terpri) (princ " is < or > or ? to")
	 (terpri) (print-term y)
	 (terpri) (princ "Answer = ")
	 (setq v (read))
	 (cond
	   ((member v '(< > ?))
	    (setq *manual-ordering-results* (acons (list x y) v *manual-ordering-results*))
	    (return v))
	   (t
...
(defun initialize-simplification-ordering-compare-equality-arguments-hash-table ()
  (setq *simplification-ordering-compare-equality-arguments-hash-table*
        (if (test-option2?)
            (make-hash-table)
            nil)))
