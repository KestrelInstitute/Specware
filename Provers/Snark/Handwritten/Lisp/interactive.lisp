...
(defun before-interactive-operation ()
  ...
  (setq *last-row-number-before-interactive-operation*
  ...
  (prog->
    ...
    (cond
     ((>= *last-row-number-before-interactive-operation* (row-number row))
      (return-from after-interactive-operation (setq it nil)))
     ((implies op (let ((reason (row-reason row)))
                    (and (consp reason) (eq op (first reason)))))
      (return-from after-interactive-operation (setq it (row-name-or-number row))))))
  (setq it nil))
