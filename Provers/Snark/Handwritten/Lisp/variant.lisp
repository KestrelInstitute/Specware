	  ...
	  (cond
	    ((null v)
	     (when (null (rassoc ,y matches))
	       (setq matches (acons ,x ,y matches))))
	    ((eq (cdr v) ,y)
	     t)))))
    (match-term
      x y subst
      :if-constant*constant (cond
			      (*extended-variant*
			       (when (and (implies *using-sorts* (same-sort-p (constant-sort x)
                                                                              (constant-sort y)))
					  (variant1 x y))
				 (funcall cc matches)))
			      ((eql x y)
      ...
      :if-compound*compound (let ((xhead (head x)) (yhead (head y)))
				...
				((and *extended-variant*
				      ...
				      (not (cons-function-symbol-p xhead))
				      (not (cons-function-symbol-p yhead))
                                      (not (equality-predicate-symbol-p xhead))
                                      (not (equality-predicate-symbol-p yhead)))
				 (when (variant1 xhead yhead)
				   (variantl cc (argsa x) (argsa y) subst matches)))
                                ((neq xhead yhead)
                                 )
                                ((cons-function-symbol-p xhead)
                                 (prog->
				 ...
				 (let ((funs (function-variant-code xhead)))
				   (if funs
					 ...
					 (funcall fun cc (argsa x) (argsa y) subst matches xhead))
				       (variantl cc (argsa x) (argsa y) subst matches))))))
      :if-variable*variable (when (and (implies *using-sorts* (same-sort-p (variable-sort x)
                                                                           (variable-sort y)))
				       (variant1 x y))
			      (funcall cc matches)))))
...
(defun variant-permute (cc terms1 terms2 subst matches fn)
  (dolist (p (function-permutations fn))
    (variantl cc terms1 (permute terms2 p) subst matches)))
