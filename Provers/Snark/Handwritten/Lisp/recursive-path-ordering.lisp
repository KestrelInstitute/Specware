  ...
  (cond
   ...
   (t
    (match-term
     ...
     :if-variable*compound (and (or (null testval) (eq '< testval))
                                (symbol-ordering-compare-variable*compound x y subst))
     :if-compound*variable (and (or (null testval) (eq '> testval))
                                (symbol-ordering-compare-compound*variable x y subst))
     :if-constant*constant (symbol-ordering-compare x y)
     :if-compound*constant (and (neq '= testval)
                                (symbol-ordering-compare-compound*constant x y subst testval))
     :if-constant*compound (and (neq '= testval)
                                (symbol-ordering-compare-constant*compound x y subst testval))
      ...
      (let ((can-be-> t))
        (dolist (yarg yargs (if can-be-> '> '?))
          (ecase (rpo-compare-terms x yarg subst nil)
            (?
             (if (eq '> testval) (return nil) (setq can-be-> nil)))
            ((< =)
             (return '<))
            (>
             ))))))
...
(defun rpo-compare-compounds? (x y xargs yargs subst testval)
  (cond
   ((and (or (null testval) (eq '> testval))
         (thereis-rpo-equal-or-greaterp xargs y subst))
    '>)
   ((and (or (null testval) (eq '< testval))
         (thereis-rpo-equal-or-greaterp yargs x subst))
    '<)
   ((null testval)
    '?)))
