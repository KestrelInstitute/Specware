...
(defun substitute-for-constant (new old x subst)
  ...
  (dereference
   ...
   :if-compound-cons (let* ((u (carc x))
                            (u* (substitute-for-constant new old u subst))
                            (v (cdrc x))
                            (v* (substitute-for-constant new old v subst)))
                       (if (and (eq u u*) (eq v v*))
                           x
                           (cons u* v*)))
   :if-compound-appl (let ((args (argsa x)))
                       (if (null args)
                           x
                           (let ((args* (substitute-for-constantl new old args subst)))
                             (if (eq args args*)
                                 x
                                 (make-compound* (heada x) args*)))))
   :if-variable x))
   ...
   :if-compound-appl (cond
		      ...
		      (t
		       (let ((args (argsa x)))
		         (if (null args)
			     x
			     (let ((args* (substitute-for-compoundl new old args subst)))
			       (if (eq args args*)
			           x
			           (make-compound* (heada x) args*)))))))))
...
(defun substitute-for-variable (new old x subst)
  ...
  (dereference
   ...
   :if-compound-nonground (let ((args (argsa x)))
		            (if (null args)
			        x
			        (let ((args* (substitute-for-variablel new old args subst)))
			          (if (eq args args*)
			              x
			              (make-compound* (heada x) args*)))))
