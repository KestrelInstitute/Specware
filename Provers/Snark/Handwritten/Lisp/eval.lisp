...

(defun wff-depth (wff &optional subst)
  (prog->
    (wff-size* wff subst :pos ->* atom subst)
    (depth atom subst)))

(defun wff-size (wff &optional subst)
  (prog->
    (wff-size* wff subst :pos ->* atom subst)
    (size atom subst)))

(defun wff-weight (wff &optional subst)
  (prog->
    (wff-size* wff subst :pos ->* atom subst)
    (weight atom subst)))

(defun wff-size+depth (wff &optional subst)
  (prog->
    (wff-size* wff subst :pos ->* atom subst)
    (+ (size atom subst) (depth atom subst))))

(defun wff-weight+depth (wff &optional subst)
  (prog->
    (wff-size* wff subst :pos ->* atom subst)
    (+ (weight atom subst) (depth atom subst))))

(defun wff-length (wff &optional subst)
  (prog->
    (wff-size* wff subst :pos ->* atom subst)
    ...
    :if-compound (let* ((head (head wff))
		   ...
		   (ecase kind
			 ...
			 (prog->
			   ...
			   (setq max (max max max1))
			   (setq min (min min min1)))
			 (values max min)))
		     (or
		       (let ((max 0) (min 0))
			 (prog->
		     ...
		     (implies
		       (prog->
			 (maximum-and-minimum-clause-lengths-neg (arg1 wff) subst -> max1 min1)
			 (maximum-and-minimum-clause-lengths (arg2 wff) subst -> max2 min2)
			 (values (+ max1 max2) (+ min1 min2))))
		     (implied-by
		       (prog->
			 (maximum-and-minimum-clause-lengths-neg (arg2 wff) subst -> max1 min1)
			 (maximum-and-minimum-clause-lengths (arg1 wff) subst -> max2 min2)
			 (values (+ max1 max2) (+ min1 min2))))
		     ((iff xor if)
		      (unimplemented))
		     ((nil)
  ...
  (dereference
    ...
    :if-compound (let* ((head (head wff))
		   ...
		   (ecase kind
		     ...
		     (implies
		       (prog->
			 (maximum-and-minimum-clause-lengths (arg1 wff) subst -> max1 min1)
			 (maximum-and-minimum-clause-lengths-neg (arg2 wff) subst -> max2 min2)
			 (values (max max1 max2) (min min1 min2))))
		     (implied-by
		       (prog->
			 (maximum-and-minimum-clause-lengths (arg2 wff) subst -> max1 min1)
			 (maximum-and-minimum-clause-lengths-neg (arg1 wff) subst -> max2 min2)
			 (values (max max1 max2) (min min1 min2))))
		     ((iff xor if)
		      (unimplemented))
		     ((nil)
