  ...
  (cond
    ((null terms)
     (let ((identity (function-identity2 fn))
       ...
       (unless (and (eq identity none) (member nil vals))
         (do ((vars vars (rest vars))
           ...
           (let ((var (first vars))
             ...
             (cond
              ((null val)
               (if (implies *using-sorts* (subsort-p (constant-sort identity) (variable-sort var)))
                   (setq subst (bind-variable-to-term var identity subst))
                   (return)))
              ((null (rest val))
               ;; already checked sort compatibility in compute-bounds
               (setq subst (bind-variable-to-term var (first val) subst)))
              (t
               ;; it would be more efficient to check sort compatibility earlier
               (if (implies *using-sorts* (subsort-p fn-sort (variable-sort var)))
                   (setq subst (bind-variable-to-term var (make-compound* fn val) subst))
                   (return)))))))))
    (t
     (let ((term (pop terms)))
  ...
  (prog->
    ...
    (cond
     ((or (> coef sum) (variable-occurs-p var term subst))
      0)
     ((or (not *using-sorts*) (function-boolean-valued-p fn))
      nil)
     (t
      ...
      (cond
       ((eq top-sort sort)
        nil)
       ((not (subsort-p (term-sort term subst) sort))
        0)
       ((not (subsort-p (associative-function-sort fn) sort))
        1)
       (t
        nil))))))

(defun subsume-bag-test1 ()
  ;; substitutions should be well sorted
  ;; they weren't guaranteed to be before 2000-07-01
  ;; in this test, x can be found to 1, 2, or 3, but not to 0 or (+ ...)
  (initialize)
  ...
  (let ((count 0))
    (prog->
      (args (arg1 (input-wff '(forall ((x :sort positive-integer)
                               ...
                               (p (+ x y z)))))
            -> terms1)
      (args (arg1 (input-wff '(p (+ 0 1 2 3)))) -> terms2)
      (subsume-bag terms1 terms2 nil (input-symbol '+) ->* subst)
      (incf count)
      (terpri)
      (dolist terms1 ->* var)
