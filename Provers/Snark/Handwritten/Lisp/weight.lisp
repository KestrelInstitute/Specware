...
(defun weight (x &optional subst)
  (dereference
   ...
   :if-compound-appl (let ((head (heada x)))
                       (cond
                        ((loop for fn in (function-weight-code head) thereis (funcall fn x subst))
                         )
                        ((function-associative head)
                         (let ((args (argsa x)))
...
(defun symbol-count (x &optional subst (scount (make-symbol-count)))
  ...
  (macrolet
    ((symbol-count1 (symbol count)
       `(let* ((count ,count)
               (alist (symbol-count-alist scount))
...
(defun symbol-count-not-greaterp (scount1 scount2)
  (and (not (> (symbol-count-total scount1)
	       (symbol-count-total scount2)))
       (let ((alist2 (symbol-count-alist scount2)))
	 (dolist (v1 (symbol-count-alist scount1) t)
	   (let ((v2 (assoc (car v1) alist2)))
	     (when (or (null v2) (> (cdr v1) (cdr v2)))
	       (return nil)))))))
