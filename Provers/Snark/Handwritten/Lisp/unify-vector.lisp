...
(defun unify-identity-with-vector (cc terms subst fn identity)
  (let ((vars nil) term)
	...
	((and (unfrozen-variable-p term)
	      (implies *using-sorts* (subsort-p (constant-sort identity) (variable-sort term))))
	 (pushnew term vars))
	(t
	 (return))))))
...
(defun unify-variable-with-vector (cc var arg args subst fn identity max)
  ...
  (when (and (implies max (<= 2 max))
             (implies *using-sorts* (subsort-p (associative-function-sort fn) (variable-sort var))))
    (let ((l nil)
          (count 0))
      (loop
        (cond
...
(defun unify-vector (cc args1 args2 subst fn &optional (identity (function-identity2 fn)))
  ...
  (prog->
    ...
    (cond
	 ...
	 (unless (eq none identity)
	   (when (implies *using-sorts* (subsort-p (constant-sort identity) (variable-sort firstargs1)))
	 ...
	 (unless (eq none identity)
	   (when (implies *using-sorts* (subsort-p (constant-sort identity) (variable-sort firstargs2)))
