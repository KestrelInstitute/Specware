                              ...
                              (let ((head (heada x)))
                                (cond
                                 ((neq head (heada y))
                                  nil)
                                 (t
                                  (let ((funs (function-equal-code head)))
                                    (if funs
                                        (dolist (fun funs nil)
                                          (when (funcall fun (argsa x) (argsa y) subst head)
                                            (return t)))
                                        (equal-list-p (argsa x) (argsa y) subst)))))))))))
...
(defun equal-bag-p (terms1 terms2 subst fn)
  (and (similar-argument-list-ac1-p fn terms1 terms2 subst)
       (progn
         (setq terms2 (cons nil (copy-list (argument-list-a1 fn terms2 subst))))
...
(defun equal-permute-p (terms1 terms2 subst fn)
  (dolist (p (function-permutations fn))
    (when (equal-p terms1 (permute terms2 p) subst)
      (return t))))
