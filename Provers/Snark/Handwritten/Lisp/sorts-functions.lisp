...
;;; (note that the external representation uses 2-element lists instead of dotted pairs)
;;;
...
(definline fsd-subsort-p (fsd sort)
  (or (eq top-sort sort) (subsort-p (fsd-result-sort fsd) sort)))
...
(defun same-fsd-p (fsd1 fsd2)
  (and (same-sort-p (fsd-result-sort fsd1) (fsd-result-sort fsd2))
       (same-sort-p (fsd-arg-sort fsd1 t) (fsd-arg-sort fsd2 t))
       (every (lambda (p) (or (eq t (car p)) (same-sort-p (cdr p) (fsd-arg-sort fsd2 (car p)))))
              (fsd-argument-sort-alist fsd1))
       (every (lambda (p) (or (eq t (car p)) (same-sort-p (cdr p) (fsd-arg-sort fsd1 (car p)))))
              (fsd-argument-sort-alist fsd2))))

(defun sub-fsd-p (fsd1 fsd2)
  (and (subsort-p (fsd-result-sort fsd1) (fsd-result-sort fsd2))
       (subsort-p (fsd-arg-sort fsd1 t) (fsd-arg-sort fsd2 t))
       (every (lambda (p) (or (eq t (car p)) (subsort-p (cdr p) (fsd-arg-sort fsd2 (car p)))))
              (fsd-argument-sort-alist fsd1))
       (every (lambda (p) (or (eq t (car p)) (supersort-p (cdr p) (fsd-arg-sort fsd1 (car p)))))
              (fsd-argument-sort-alist fsd2))))
  ...
  (let (result-sort default-sort (arg-sort-alist nil))
    (cond
     ...
     ((null fsd2)
      fsd1)
     ((eq bottom-sort
          (setf default-sort (sort-intersection (fsd-arg-sort fsd1 t) (fsd-arg-sort fsd2 t))))
      none)
     ...
     ((eq bottom-sort
          (setf result-sort (sort-intersection (fsd-result-sort fsd1) (fsd-result-sort fsd2))))
      :error)
     (t
      (unless (eq top-sort default-sort)
...
(defun input-argument-sort-alist (function l)
  (cond
   ...
   ((can-be-argument-sort-alist-p1 function l)
    (mapcar (lambda (p) (cons (first p) (input-sort2 (second p)))) l))
   ((can-be-argument-sort-alist-p2 function l)
    (let ((i 0)) (mapcar (lambda (s) (cons (incf i) (input-sort2 s))) l)))
   (t
  ...
  (and (consp l)
       (let* ((arity (function-arity function))
              (can-be-key-p (cond
                             ...
                             (t
                              (ecase arity
                                (:alist #'can-be-alist-key-name-p)
                                (:plist #'can-be-plist-key-name-p)
                                ((:any :list*) #'positive-integer-p))))))
         (dotails (l l t)
           (let ((p (first l)))
             (unless (and (consp p)
                          (if (eq t (first p))
                          ...
                          (consp (rest p))
                          (null (rest (rest p)))
                          (or (eq top-sort (second p))
                              (can-be-sort-name-or-conjunction-p (second p))))
               (return nil)))))))

(defun can-be-argument-sort-alist-p2 (function l)
  (and (consp l)
       (let ((arity (function-arity function)))
         (and (or (naturalp arity) (eq :any arity) (eq :list* arity))
              (every (lambda (s)
                       (or (eq top-sort s)
                           (can-be-sort-name-or-conjunction-p s)))
                     l)))))
