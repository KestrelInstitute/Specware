  ...
  (let ((simple-solution (make-array (list nxcoefs nycoefs)))	;x-term.i,y-term.j unifiability
    ...
    (loop for i below nxcoefs
	  ...
	  do (incf suma coef)
		 ...
		 (unless ground
		   (setq new-all-x-term-ground nil)))))
    (loop for j below nycoefs
	  ...
	  do (incf sumb coef)
		 ...
		 (unless ground
		   (setq new-all-y-term-ground nil)))))
    (setq all-x-term-ground new-all-x-term-ground)
    (setq all-y-term-ground new-all-y-term-ground)
    (when (cond
	    (all-x-term-ground
    ...
    (let ((xx-and-yy-unify-array (let ((ncoefs (if (>= nxcoefs nycoefs) nxcoefs nycoefs)))
      ...
      (unless all-x-term-ground
	(dotimes (i (1- nxcoefs))		;initialize xx-unify-p
      ...
      (unless all-y-term-ground
	(dotimes (j (1- nycoefs))		;initialize yy-unify-p
      ...
      (setq x-term-ground-array nil)		;done with x-term-ground-array
      (setq y-term-ground-array nil)		;and y-term-ground-array now
...

(declare-snark-option2 use-subsume-bag t)
    ...
    ((meter-unify-bag?)
	 ...
	 (let ((elapsed-time (/ (- (get-internal-run-time) start-time)
	   ...
	   (when (implies (numberp (meter-unify-bag?)) (<= (meter-unify-bag?) elapsed-time))
	     (format t "~2&~,3F seconds to unify-bag " elapsed-time)
	     (print-term (flatten-term (make-compound* fn terms1) subst))
	     (format t " and ")
	     (print-term (flatten-term (make-compound* fn terms2) subst))
	     (format t "."))))))
    (t
     (unify-bag* cc fn terms1 terms2 subst))))
    ...
    (loop for tc in terms-and-counts
	  ...
	  when (plusp count)
	    do (incf nxcoefs)
	       (unless firsta
		 (setq firsta count)
		 (setq firstx (tc-term tc)))
	       (when (or (not x-term-is-ground) all-x-term-ground)
		 (if (frozen-p (tc-term tc) subst)
		     (setq x-term-is-ground t)
		     (setq all-x-term-ground nil)))
	  else
	  when (minusp count)
	    do (incf nycoefs)
	       (unless firstb
		 (setq firstb (- count))
		 (setq firsty (tc-term tc)))
	       (when (or (not y-term-is-ground) all-y-term-ground)
		 (if (frozen-p (tc-term tc) subst)
		     (setq y-term-is-ground t)
		     (setq all-y-term-ground nil))))
    (cond
  ...
  (labels
    ((unify-bag2 (complex-solns subst)
	   ...
	   ((consp simple-solution.i.j)
		...
		((and (or (and (null x-bind.i) (eql 1 m))
		 ...
		 (UNLESS (AND (NEQ NONE IDENTITY)		;AC1 UNIFICATION SUPPORT
		     ...
		     (unless (or (nosol3x j+1) (nosol3y (1+ i)))
		       (unify-bag3* i j+1 subst))))
		 (cond
     ...
     (print-bindings (term bind ncoefs)
       (dotimes (i ncoefs)
	 (terpri-comment)
	 (print-term (svref term i))
	 (princ " & ")
	 (print-term (make-a1-compound* fn identity (svref bind i))))))
		   ...
		   ((and (not all-x-term-ground) (plusp count))
			...
			(dereference
			  ...
			  :if-variable (if (variable-frozen-p term)
					   (setq x-has-nonvar t)
					   (setq x-has-var t))
			  :if-constant (setq x-has-nonvar t)
			  :if-compound (setq x-has-nonvar t)))))
		   ((and (not all-y-term-ground) (minusp count))
			...
			(dereference
			  ...
			  :if-variable (if (variable-frozen-p term)
					   (setq y-has-nonvar t)
					   (setq y-has-var t))
			  :if-constant (setq y-has-nonvar t)
			  :if-compound (setq y-has-nonvar t))))))))
...

(defun unify-bag-test1 (&optional unify-bag-constant-abstraction test-option8)
  (initialize)
...
(defun unify-bag-test2 (&optional unify-bag-constant-abstraction test-option8)
  (initialize)
  (use-unify-bag-constant-abstraction unify-bag-constant-abstraction)
  (test-option8 test-option8)
  (meter-unify-bag)
